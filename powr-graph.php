<?php
    ...
    Plugin URI: https://www.powr.io/plugins/graph
    ...
    */
    ...
    class Powr_Graph extends WP_Widget{
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Graph.
        </p>
        <p>
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_graph_menu' );
    function powr_graph_menu() {
      add_menu_page( 'POWr Graph', 'POWr Graph', 'manage_options', 'powr-graph-settings', 'powr_graph_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_graph_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/graph';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/graph';
      }
    ...
    register_activation_hook(__FILE__, 'powr_graph_plugin_activate');
      add_action('admin_init', 'powr_graph_plugin_redirect');

      function powr_graph_plugin_activate() {
      add_option('powr_graph_plugin_do_activation_redirect', true);
      }
      ...
      function powr_graph_plugin_redirect() {
      if (get_option('powr_graph_plugin_do_activation_redirect', false)) {
          delete_option('powr_graph_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-graph-settings&' );
          }
       }
      }
